  ...
  <div :style="toolStyle">
    ...
    <el-menu default-active="" class="el-menu-vertical" :collapse="isCollapse" v-for="(item,index) in toolBtns" :key="index">
      ...
      <el-submenu v-if="item.dic.length!==0" :index="index+1+''" :hide-timeout="0" :show-timeout="0">
        ...
        <!-- 在空的未知 -->
        <!-- 下面的高度因为有border的问题所以需要修改下 -->
        <el-menu-item-group v-for="(dicItem,idx) in item.dic" :key="idx" :style="dicStyle">
          <!-- 这是分组的说明 -->
          <span slot="title">{{dicItem.group}}</span>

          <!-- gutter是间距 -->
          <el-row :gutter="12" style="padding: 8px 12px;" @click="clickItem">
            <el-col :span="24" v-for="(groupItem,groupIdx) in dicItem.groupItems" :key="(index+1)+'-'+(groupIdx+1)">
