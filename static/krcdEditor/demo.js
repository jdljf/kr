window.onload = function() {
  var krcd = window.krcd = new KRCD({
    el: document.querySelector('#krcd'),
    ...
    ctrl_remote_handle: function(data) {
      //这里可以处理url，对url进行再加工。如果此时执行 this.isLoadAsyncData(true)，则表示代替krcd自带的异步请求方法，
    ...
    default_open_toolbar: 'krcd-toolbar-tools', //默认打开的toolbar的集合，如果不填，默认使用第一个集合
  });
  var krcd1 = new KRCD({
    el: document.querySelector('#krcd1'),
    iframe_js_src: 'demoforeditor.js',
  });
  krcd.addListener('ready', function() {
    // krcd.execCommand('seniorprint', {
    ...
    //krcd.mode('EDITOR'); //执行到这一行时 krcd对象才加载成功！
    console.log(this);
    console.log('design ready ok!');
  });
  krcd.addListener('headerfooteropen', function() {
    console.log(this);
    console.log('design headerfooteropen ok!');
  });

  krcd.addListener('beforerender', function() {
    //debugger;
    console.log('beforerender ok!');
  });
  krcd.addListener('rendered', function() {
    console.log(arguments);
    //debugger;
    console.log('rendered ok!');
  });
  // krcd.addListener('loaddata', function(controls, error) {
  //   debugger;
  //   console.log('loaddata ok!');
  // });
  krcd.addListener('click', function() {
    console.log(arguments);
    console.log('click ok!');
  });
  // krcd.addListener('openform', function() {
  //   debugger;
  //   console.log('openform ok!');
  // });
  krcd.addListener('valuechange', function() {
    console.log(arguments)
    console.log('valuechange ok!');
  });
  // krcd.addListener('closeform', function() {
  //   debugger;
  //   console.log('closeform ok!');
  // });

  krcd.addListener('contentchange', function() {
