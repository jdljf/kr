  ...
  scrawl.prototype = {
    ...
    _addClearSelectionListenter: function() {
      ...
      domUtils.on(doc, 'mousemove', function(e) {
        ...
        if (e.target.nodeName === 'INPUT') { //chrome中防止input无法点击输入
...
function ue_callback(url, state) {
  ...
  if (state == "SUCCESS") {
    ...
    img.onload = function() {
      ...
      if (picBorard.children.length === 0) {
...
function exec(scrawlObj) {
  if (scrawlObj.isScrawl) {
    ...
    if (scrawlObj.isEditCanvas) {
      UE.plugins["krcdscrawl"].editdom.src = 'data:image/png;base64,' + base64;
      delete UE.plugins['krcdscrawl'].editdom;
    } else {
      var img = document.createElement('img');
      img.setAttribute('src', 'data:image/png;base64,' + base64);
      img.classList.add("krcd-scrawl"); //刚刚改了这里，明天需要把样式加一下、plugin加一下
